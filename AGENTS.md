...
- **4k ctx.** 4096-token window (1.5b preset); the per-model pruner budget, message cap, and tool-result cap (`max_tool_result_bytes`) come from the calibration table (`core/context.py:28-34`), applied in `plugins/agent/loop.py` `start()`; the 1.5b budget = 3000 leaves 1k KV headroom (core fallback default `core/context_pruner.py:26`). Folds trigger when `tokens > pruner_budget OR messages > max_messages` (`plugins/agent/loop.py:388`); the pruner enforces BOTH metrics — count pass + token pass that drops lowest-score messages until within budget even under the message cap (`core/context_pruner.py` `prune`); a single tool result is truncated to its calibrated byte cap so one result can never swallow the window (`plugins/agent/loop.py` `_record_tool_result`).
...
- **Zero-token guarantee.** Any new router/dispatch must be deterministic (regex/sympy), adding zero LLM tokens in `lite` profile. Never route a query through an LLM unless gated behind `--enable-semantic-router` and only in `full` profile — applies to the semantic router (`plugins/agent/loop.py:324-329`, `plugins/agent/semantic_router.py:37-38,54`, `main.py:108,112`) AND the multi-domain unresolved-fragment fallback (`plugins/agent/loop.py:269-277`).
...
- **Single-pruner preservation.** `assistant` messages with `tool_calls` MUST be preserved for 1.5B coherence (`core/context_pruner.py:26`, `core/summarizer.py:64-65`); never let a second pruner delete reasoning (`plugins/agent/loop.py:384-391`).
- **Injection hardening.** Prompt/context injections are injected as `user` with the `[injected context]` prefix, NEVER `system` (`plugins/agent/loop.py:421-427`).
- **Event hygiene.** Exactly one `turn.start` (outer) + `turn.round` per iteration; no duplicate emits (`plugins/agent/loop.py:312,414`).
